<?php
/**
 * صفحة الجدول الدراسي للطالب
 */

// استيراد ملفات الإعدادات والدوال
require_once '../includes/config.php';
require_once '../includes/auth.php';

// التحقق من تسجيل دخول الطالب
if (!isLoggedIn() || $_SESSION['user_type'] !== 'student') {
    header('Location: ../login.php');
    exit;
}

// الحصول على معلومات الطالب
$student_id = $_SESSION['user_id'];
$student_name = $_SESSION['user_name'];

// الحصول على الجدول الدراسي
$schedule = [];
try {
    $query = "SELECT s.*, c.name as course_name, c.code as course_code,
                     u.first_name as teacher_first_name, u.last_name as teacher_last_name
              FROM schedules s
              JOIN courses c ON s.course_id = c.id
              JOIN enrollments e ON c.id = e.course_id
              LEFT JOIN users u ON c.teacher_id = u.id
              WHERE e.student_id = :student_id
              ORDER BY s.day_of_week, s.start_time";
    $stmt = $pdo->prepare($query);
    $stmt->execute(['student_id' => $student_id]);
    $schedule = $stmt->fetchAll();
} catch (Exception $e) {
    $schedule = [];
}

// تنظيم الجدول حسب الأيام
$days = [
    1 => 'الأحد',
    2 => 'الاثنين', 
    3 => 'الثلاثاء',
    4 => 'الأربعاء',
    5 => 'الخميس',
    6 => 'الجمعة',
    7 => 'السبت'
];

$weekly_schedule = [];
foreach ($days as $day_num => $day_name) {
    $weekly_schedule[$day_num] = [
        'name' => $day_name,
        'classes' => []
    ];
}

foreach ($schedule as $class) {
    $weekly_schedule[$class['day_of_week']]['classes'][] = $class;
}
?>
<!DOCTYPE html>
<html lang="ar" dir="rtl">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?php echo SITE_NAME; ?> - الجدول الدراسي</title>
    
    <!-- Bootstrap RTL -->
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.rtl.min.css">
    
    <!-- Font Awesome -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    
    <!-- خط Cairo -->
    <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Cairo:wght@300;400;500;600;700&display=swap">
    
    <style>
        body {
            font-family: 'Cairo', sans-serif;
            background-color: #f8f9fa;
        }
        .sidebar {
            background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
            min-height: 100vh;
            color: white;
        }
        .sidebar .nav-link {
            color: rgba(255,255,255,0.8);
            border-radius: 8px;
            margin: 2px 0;
        }
        .sidebar .nav-link:hover,
        .sidebar .nav-link.active {
            background-color: rgba(255,255,255,0.1);
            color: white;
        }
        .card {
            border: none;
            border-radius: 15px;
            box-shadow: 0 4px 6px rgba(0,0,0,0.1);
        }
        .schedule-table th {
            background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
            color: white;
            text-align: center;
        }
        .class-card {
            background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
            color: white;
            border-radius: 10px;
            padding: 10px;
            margin: 5px 0;
            text-align: center;
        }
        .time-slot {
            font-weight: bold;
            color: #495057;
        }
    </style>
</head>
<body>
    <div class="container-fluid">
        <div class="row">
            <!-- الشريط الجانبي -->
            <div class="col-md-3 col-lg-2 sidebar p-3">
                <div class="text-center mb-4">
                    <h4><?php echo SITE_NAME; ?></h4>
                    <p class="mb-0">مرحباً، <?php echo htmlspecialchars($student_name); ?></p>
                </div>
                
                <nav class="nav flex-column">
                    <a class="nav-link" href="dashboard.php">
                        <i class="fas fa-tachometer-alt me-2"></i>
                        لوحة التحكم
                    </a>
                    <a class="nav-link" href="student_courses.php">
                        <i class="fas fa-book me-2"></i>
                        المقررات
                    </a>
                    <a class="nav-link" href="student_assignments.php">
                        <i class="fas fa-tasks me-2"></i>
                        الواجبات
                    </a>
                    <a class="nav-link" href="student_grades.php">
                        <i class="fas fa-chart-line me-2"></i>
                        الدرجات
                    </a>
                    <a class="nav-link active" href="student_schedule.php">
                        <i class="fas fa-calendar me-2"></i>
                        الجدول الدراسي
                    </a>
                    <a class="nav-link" href="student_notifications.php">
                        <i class="fas fa-bell me-2"></i>
                        الإشعارات
                    </a>
                    <a class="nav-link" href="student_messages.php">
                        <i class="fas fa-envelope me-2"></i>
                        الرسائل
                    </a>
                    <a class="nav-link" href="student_forums.php">
                        <i class="fas fa-comments me-2"></i>
                        المنتديات
                    </a>
                    <a class="nav-link" href="student_profile.php">
                        <i class="fas fa-user me-2"></i>
                        الملف الشخصي
                    </a>
                    <a class="nav-link" href="student_settings.php">
                        <i class="fas fa-cog me-2"></i>
                        الإعدادات
                    </a>
                    <hr>
                    <a class="nav-link" href="../logout.php">
                        <i class="fas fa-sign-out-alt me-2"></i>
                        تسجيل الخروج
                    </a>
                </nav>
            </div>
            
            <!-- المحتوى الرئيسي -->
            <div class="col-md-9 col-lg-10 p-4">
                <!-- رأس الصفحة -->
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h1 class="h3">
                        <i class="fas fa-calendar me-2"></i>
                        الجدول الدراسي
                    </h1>
                    <div>
                        <span class="badge bg-primary"><?php echo count($schedule); ?> محاضرة</span>
                    </div>
                </div>
                
                <?php if (empty($schedule)): ?>
                    <!-- رسالة عدم وجود جدول -->
                    <div class="card">
                        <div class="card-body text-center py-5">
                            <i class="fas fa-calendar fa-3x text-muted mb-3"></i>
                            <h4 class="text-muted">لا يوجد جدول دراسي</h4>
                            <p class="text-muted">لم يتم إنشاء جدول دراسي لك حتى الآن. يرجى التواصل مع إدارة الكلية.</p>
                        </div>
                    </div>
                <?php else: ?>
                    <!-- الجدول الأسبوعي -->
                    <div class="card">
                        <div class="card-header">
                            <h5 class="card-title mb-0">
                                <i class="fas fa-table me-2"></i>
                                الجدول الأسبوعي
                            </h5>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-bordered schedule-table">
                                    <thead>
                                        <tr>
                                            <th style="width: 100px;">الوقت</th>
                                            <?php foreach ($days as $day_name): ?>
                                                <th><?php echo $day_name; ?></th>
                                            <?php endforeach; ?>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php
                                        // إنشاء فترات زمنية من 8 صباحاً إلى 6 مساءً
                                        $time_slots = [];
                                        for ($hour = 8; $hour <= 18; $hour++) {
                                            $time_slots[] = sprintf('%02d:00', $hour);
                                        }
                                        
                                        foreach ($time_slots as $time_slot):
                                        ?>
                                            <tr>
                                                <td class="time-slot"><?php echo $time_slot; ?></td>
                                                <?php foreach ($days as $day_num => $day_name): ?>
                                                    <td>
                                                        <?php
                                                        // البحث عن محاضرة في هذا الوقت واليوم
                                                        foreach ($weekly_schedule[$day_num]['classes'] as $class) {
                                                            $start_time = date('H:i', strtotime($class['start_time']));
                                                            if ($start_time === $time_slot) {
                                                                $end_time = date('H:i', strtotime($class['end_time']));
                                                                echo '<div class="class-card">';
                                                                echo '<strong>' . htmlspecialchars($class['course_name']) . '</strong><br>';
                                                                echo '<small>' . htmlspecialchars($class['course_code']) . '</small><br>';
                                                                echo '<small>' . $start_time . ' - ' . $end_time . '</small><br>';
                                                                if ($class['teacher_first_name']) {
                                                                    echo '<small>د. ' . htmlspecialchars($class['teacher_first_name'] . ' ' . $class['teacher_last_name']) . '</small><br>';
                                                                }
                                                                if ($class['room']) {
                                                                    echo '<small>قاعة: ' . htmlspecialchars($class['room']) . '</small>';
                                                                }
                                                                echo '</div>';
                                                                break;
                                                            }
                                                        }
                                                        ?>
                                                    </td>
                                                <?php endforeach; ?>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                    
                    <!-- قائمة المحاضرات -->
                    <div class="row mt-4">
                        <div class="col-md-6">
                            <div class="card">
                                <div class="card-header">
                                    <h5 class="card-title mb-0">
                                        <i class="fas fa-list me-2"></i>
                                        محاضرات اليوم
                                    </h5>
                                </div>
                                <div class="card-body">
                                    <?php
                                    $today = date('N'); // 1 = Monday, 7 = Sunday
                                    $today_classes = $weekly_schedule[$today]['classes'] ?? [];
                                    ?>
                                    
                                    <?php if (empty($today_classes)): ?>
                                        <div class="alert alert-info">
                                            <i class="fas fa-info-circle me-2"></i>
                                            لا توجد محاضرات اليوم
                                        </div>
                                    <?php else: ?>
                                        <?php foreach ($today_classes as $class): ?>
                                            <div class="border-bottom pb-2 mb-2">
                                                <h6><?php echo htmlspecialchars($class['course_name']); ?></h6>
                                                <small class="text-muted">
                                                    <?php echo date('H:i', strtotime($class['start_time'])); ?> - 
                                                    <?php echo date('H:i', strtotime($class['end_time'])); ?>
                                                    <?php if ($class['room']): ?>
                                                        | قاعة: <?php echo htmlspecialchars($class['room']); ?>
                                                    <?php endif; ?>
                                                </small>
                                            </div>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                        
                        <div class="col-md-6">
                            <div class="card">
                                <div class="card-header">
                                    <h5 class="card-title mb-0">
                                        <i class="fas fa-clock me-2"></i>
                                        المحاضرة القادمة
                                    </h5>
                                </div>
                                <div class="card-body">
                                    <?php
                                    // البحث عن المحاضرة القادمة
                                    $current_time = date('H:i');
                                    $next_class = null;
                                    
                                    foreach ($today_classes as $class) {
                                        $start_time = date('H:i', strtotime($class['start_time']));
                                        if ($start_time > $current_time) {
                                            $next_class = $class;
                                            break;
                                        }
                                    }
                                    ?>
                                    
                                    <?php if ($next_class): ?>
                                        <div class="text-center">
                                            <h5><?php echo htmlspecialchars($next_class['course_name']); ?></h5>
                                            <p class="text-muted"><?php echo htmlspecialchars($next_class['course_code']); ?></p>
                                            <div class="badge bg-primary p-2">
                                                <?php echo date('H:i', strtotime($next_class['start_time'])); ?> - 
                                                <?php echo date('H:i', strtotime($next_class['end_time'])); ?>
                                            </div>
                                            <?php if ($next_class['room']): ?>
                                                <p class="mt-2">
                                                    <i class="fas fa-map-marker-alt me-1"></i>
                                                    قاعة: <?php echo htmlspecialchars($next_class['room']); ?>
                                                </p>
                                            <?php endif; ?>
                                        </div>
                                    <?php else: ?>
                                        <div class="alert alert-info text-center">
                                            <i class="fas fa-info-circle me-2"></i>
                                            لا توجد محاضرات قادمة اليوم
                                        </div>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
        </div>
    </div>
    
    <!-- Bootstrap JS -->
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>

